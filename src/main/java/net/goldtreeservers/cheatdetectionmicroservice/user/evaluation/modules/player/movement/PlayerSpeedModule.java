...
import fi.joniaromaacheatdetectionmicroserviceminecrafthook.network.outgoing.IExplosionOutgoingPacket;
import net.goldtreeservers.cheatdetectionmicroservice.minecraft.utils.MathHelper;
...
import net.goldtreeservers.cheatdetectionmicroservice.user.evaluation.violations.player.movement.InvalidMovement;
...
public class PlayerSpeedModule extends GameStatusAwareSubmodule
{
	...
	{
		return (float)this.getModule().getAttributes().getAttributeInstance(this.getEvaluation().getUser().getHook().getGameHook().getEntityHook().getAttributeHook().getSharedMovementSpeedAttribute()).getAttributeValue();
	}
