...
public class MinecraftPacketManager
{
	...
	public IMinecraftIncomingPacket readIncomingPacket(IPacketBuffer buffer)
	{
		...
		try
		{
			IMinecraftPacket packet = this.hook.getNetworkHook().getNetworkState(NetworkState.PLAY).createServerboundPacket(packetId);
	...
	public IMinecraftOutgoingPacket readOutgoingPacket(IPacketBuffer buffer)
	{
		...
		try
		{
			IMinecraftPacket packet = this.hook.getNetworkHook().getNetworkState(NetworkState.PLAY).createClientboundPacket(packetId);
	...
	public static MinecraftPacketManager getPacketManager(int protocolVersion, boolean inspect)
	{
		...
		{
			IMinecraftHook hook = MinecraftHooks.getHook(k);
			if (hook == null)
			{
				return null;
			}
